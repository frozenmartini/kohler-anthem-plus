...
⚠️ **The directory in a quoted path is the one that was current when that page was
written, and older pages are left as they stand.** The developer capture folder moved on
2026-09-15, out of `/config/kohler_anthem_plus_raw/`; case studies and reference pages
written before that still name the old location. The **filename** identifies a record — the
directory does not.
...
| [`protected-assets/project/`](../protected-assets/project/) | Project-created README branding and documentation illustrations: the hero mark and the hand-drawn hardware diagrams. These are separately protected; see [`NOTICE.md`](../NOTICE.md). |
| [`protected-assets/screenshots/`](../protected-assets/screenshots/) | Protected repository copies of Konnect and Home Assistant screenshots containing third-party interfaces. Their respective owners' rights are not claimed by this project. |
| [`images/`](images/) | Small utility graphics used only to lay out the README. These remain MIT licensed. |
...
| Development tests | Maintained in the author's separate local workbench and not distributed in this public repository. The offline runner uses an explicit allowlist because the private harness also contains live and water-capable probes. |
