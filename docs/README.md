...
| `images/` | Artwork for the root README and [`user_guide.md`](user_guide.md): **`banner-mark.svg`** (the hero — the v27 mark with its real knock-out mask, at `width`/`height` 1024, roughly 13x the 79-unit `viewBox`; verified 2026-09-07 with `rsvg-convert` that a mask rendered directly at target resolution is exactly as crisp as a plain fill/stroke, so the generous declared size — not repainting the mask away — is what keeps it clean if some consumer bakes a bitmap at its "natural size" before scaling. **`banner-mark-solid.svg`** is the fallback this file carried until then — the same mark with the mask repainted as white lines, immune to any masking quirk by construction since it has no mask; swap the `<img>` tags back to it if the masked version ever bands), the Konnect and Home Assistant screenshots, the four drawn hardware icons plus **`hw-combo.svg`** (the three-device assembly drawn as one file, so it scales instead of wrapping), and the **`vline-grey-*.svg`** column rules — one file per height, because an `<img>` cannot stretch to its row. |
