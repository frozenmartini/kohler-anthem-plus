...
- **Anthem Plus** (SKU `HUB`) — a Linux system controller that drives the valves *and*
  ...
  organised around **favorites** rather than direct commands.
...
| `kohler-work/tests/…`, `run_offline.sh` | The **external** regression harness, outside this repo. Not the in-repo `tests/`, which holds the entity suite and runs with `pytest`. |
| `captures/…` | Archived evidence from specific incidents, kept with the workspace above. |

> **The in-repo `tests/` is a different thing.** Since 2026-09-10 this repository has its own
> suite at `tests/`, run with `python3 -m pytest tests/` from the repository root and in CI on
> every push. It constructs every entity on a simulated valve and asserts the resulting set —
> the check that was missing when 0.6.6 shipped with two entities deleted. Any `test_*.py`
> named in the protocol documents below belongs to the **external** harness and is not a path
> in this repository.
...
| Path | Covers |
|---|---|
| [`user_guide.md`](user_guide.md) | **The full user guide** — every entity, the `custom_shower` and `send_valve_hex` actions, each feature in detail, automation examples, troubleshooting. This was the repository README until 2026-08-22; the root README is now a short overview that points here. |
...
| `images/` | Artwork for the root README and [`user_guide.md`](user_guide.md): **`banner-mark.png`** (the hero — a 300×300 raster, displayed at 150 px rendered directly from the same masked source as the brand icon, `brand/src/icon-v27.svg`, at exactly the size the README displays it. Two earlier attempts at a live SVG mask — a 316px and then a 1024px declared `width`/`height` on a `banner-mark.svg` — both still pixelated on GitHub's actual renderer, disproving the theory that its bake resolution follows the file's declared size. A PNG baked once at the real display resolution sidesteps that renderer entirely: there's nothing left for it to rasterise), the Konnect and Home Assistant screenshots, the four drawn hardware icons plus **`hw-combo.svg`** (the three-device assembly drawn as one file, so it scales instead of wrapping), and the **`vline-grey-*.svg`** column rules — one file per height, because an `<img>` cannot stretch to its row. |
| [`releases/`](releases/) | One file per released version, named for its tag — the notes that become that release's description on GitHub. Written and committed **before** the tag is cut, so `.github/workflows/release.yml` can draft the release from the file rather than anyone pasting it by hand. They matter beyond GitHub: HACS builds Home Assistant's update-dialog notes from the release description, so a release with an empty body tells users nothing. |
...
| HUB reports `valve2: NotConnected`, `light`/`steam` `NotConnected` | Lighting and steam favorite fields are **decompile-mapped only**, never exercised |
