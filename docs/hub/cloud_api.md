...
| Hub water usage | `/devices/api/{version}/device-management/hub-usage/{deviceId}?FromDate=&ToDate=&Interval=` | [STATIC] `AnthemHubWaterUsageModel` — same three PascalCase queries as `gcs-usage` (untested here), see [`../gcs/api.md`](../gcs/api.md#gcs-usage--per-period-water-usage-the-series-behind-the-apps-chart) |
