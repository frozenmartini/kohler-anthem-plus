...
| finding | where |
|---|---|
| **A `0x40` pause left alone is held for ~120 s, then torn down** — the session ends and the setpoint reverts to `defaultTemp`. **40 of 61** natural teardowns land at 119.651–120.724 s, on every start route. ⚠️ The remaining 21 are faster — 11 inside 3 s — and **nothing predicts which**: not the zone, not the other zone's state, not the date. | [6 §5c](06_two_cutoffs_and_a_prediction_that_failed.md), [5 §9](05_three_restarts_and_the_unexplained_00.md), [4 §5](04_two_touchscreens_and_what_off_means.md) |
