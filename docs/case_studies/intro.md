...
| finding | where |
|---|---|
| ⭐ **A shower session ends ~120 s after ALL water stops** — not when the water stops, and nothing to do with pauses. The setpoint reverts to `defaultTemp` and the Anthem Plus screen shows its totals and goes dark. Measured 118.68–121.09 s over six clean cases, **including three showers containing no `0x40` at all**. The valve ends its own session separately, at `0x40/0x40`, with its own summary screen — **two products, two independent session clocks**. This supersedes the earlier reading of the same ~120 s measurements as a pause self-terminating. | [7 §6](07_the_controller_sweeps.md), [4 §5](04_two_touchscreens_and_what_off_means.md) |
...
## 6b. ⭐ Conclusions — start here for max shower duration

[`conclusions.md`](conclusions.md) synthesises all seven case studies into one answer to
"how does max shower duration actually work, and what is broken": both devices' settings,
signals and measured timing, the controller's sweep and its zone-1 disarm bug, and the six
distinct controller faults with the log figures behind each. Every claim names the case study
it comes from. **Read it before re-deriving any of this.**
...
| **[6](06_two_cutoffs_and_a_prediction_that_failed.md)** | 2026-08-18 evening, two cutoffs 136.7 s apart, both maximums 900 s, first shower on the `0x40`-required code | **The matched-durations fix confirmed live** — both zones cut at 900 s, both carried `0x40`, both restored, flow and temperature byte-exact; the controller's `0x00` landed +0.323 s behind the valve's pause. **Per-zone clocks confirmed to 0.013 s.** A restore does not re-anchor the other zone's hardware timer. Then records a **failed prediction**: three successive theories for when a `0x40` becomes `0x00`, all built on **58 of the corpus's 90 files**, none surviving the full set. ⚠️ **The corpus spans five directories and two schemas — print its range before trusting any corpus claim.** |
| **[7](07_the_controller_sweeps.md)** | 2026-08-19, three showers **plus a four-leg designed experiment**, **HUB 15 min / GCS 60 min** — the controller as sole authority. Firmware **app 2.88 / OS 5.4** | ⭐ **The controller's cutoff is a software SWEEP, and the sweep has a bug.** It arms a timer for every zone it sees start — **the start route is irrelevant** — and when one fires it cuts every zone at or over the limit. **But a zone-2 sweep silently DISARMS zone 1's timer**, one-directionally, so zone 1 loses its maximum-duration protection entirely until a later sweep catches it over the limit — it overran by **13:05, 10:32 and 11:07**. Simulated across five sessions: **14 of 14 controller stops, zero false alarms**, and it derives case study 5's `00/00` from the rule. The stop is `0x00`, never `0x40`, always late (**+900.30 to +901.25 s**, 11 of 11). Also establishes that **a session ends after the water stops, not at it**, and that the ⚠️ mismatch WARNING cannot fire in the mismatched case. |
