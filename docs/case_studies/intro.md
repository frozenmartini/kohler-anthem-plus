...
| **[7](07_the_controller_sweeps.md)** | 2026-08-19, three showers **plus a four-leg designed experiment**, **HUB 15 min / GCS 60 min** — the controller as sole authority. Firmware **app 2.88 / OS 5.4** | ⭐ **The controller's cutoff is a software SWEEP, and the sweep has a bug.** It arms a timer for every zone it sees start — **the start route is irrelevant** — and when one fires it cuts every zone at or over the limit. **But a zone-2 sweep silently DISARMS zone 1's timer**, one-directionally, so zone 1 loses its maximum-duration protection entirely until a later sweep catches it over the limit — it overran by **28:05, 10:32 and 11:07**. Simulated across five sessions: **14 of 14 controller stops, zero false alarms**, and it derives case study 5's `00/00` from the rule. The stop is `0x00`, never `0x40`, always late (**+900.30 to +901.25 s**, 11 of 11). Also establishes that **a session ends after the water stops, not at it**, and that the ⚠️ mismatch WARNING cannot fire in the mismatched case. |
