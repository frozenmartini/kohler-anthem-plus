...
| **[7](07_the_controller_sweeps.md)** | 2026-08-19, three showers, **HUB 15 min / GCS 60 min** — the controller as sole authority, `valveOnOff` start, touchscreen control | ⭐ **The controller's cutoff is a software SWEEP, not a countdown per zone.** It arms timers on events; when one fires it checks every zone and cuts each one at or over the limit — 15 of 16 decisions. So a zone whose timer never armed can run **28 minutes past a 15-minute limit** and is only cut as collateral. Zone 2 armed 6/6, zone 1 **1 of 4**. The stop is `0x00`, never `0x40`, and always **late: +0.30 to +1.16 s**. Resolves case study 5's `00/00` with a mechanism, undoes case study 3's retraction, and shows **the session ends ~120 s after all water stops**, not at the pause. ⚠️ Also: the mismatch WARNING cannot fire in the mismatched case. |
